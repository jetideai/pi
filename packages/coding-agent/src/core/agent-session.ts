...
export class AgentSession {
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				persistedEntryId = this.sessionManager.appendMessage(event.message, entryId);
			}
			if (persistedEntryId) this._entryIdsByMessage.set(event.message, persistedEntryId);
	...
	private async _prompt(
		...
		if (this._isEmittingAgentSettled) {
			this._deferredSettledActions.push(async () => await this._prompt(text, options, initiator));
			return;
		}
		...
		try {
			...
			if (!processedInput) {
				if (ownsRunAdmission) this._finishPromptPreflight(true);
				preflightResult?.(true);
				return;
			}
